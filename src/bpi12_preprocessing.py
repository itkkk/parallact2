...
from libraries import DeepInsight_train_norm
...
def preprocess(dataset, window_size = 3):
    ...
    for fold in range(3):
        ...
        columns = ["CaseID", "Activity", "Resource", "Timestamp", "LoanAmount"]
        ...
        train_caseID_matrix, train_features_matrix, train_time_matrix, train_resource_matrix, train_target_matrix = create_dictionaries(train)
        test_caseID_matrix, test_features_matrix, test_time_matrix, test_resource_matrix, test_target_matrix = create_dictionaries(test)
        print(".", end='')

        # riordino le colonne del test set
        test_features_matrix = reorder_df_columns(train_features_matrix, test_features_matrix)
        test_target_matrix = reorder_df_columns(train_target_matrix, test_target_matrix)
...
def chain_dataset(dataset, window_size):
    ...
    for caseID, row in grouped.iterrows():
        ...
        timestamp_diff_list = (list(int(round((datetime.strptime(timestamp, TIME_FORMAT) - datetime.strptime(timestamp_list[0], TIME_FORMAT)).total_seconds() / 60)) for timestamp in timestamp_list))

        timestamp_diff_list[:] = [x / max(timestamp_diff_list) if max(timestamp_diff_list) > 0 else 0 for x in timestamp_diff_list]
        ...
        for i in range(1, len(timestamp_list)):
            timestamp_local_diff_list.append(
                int(round((datetime.strptime(timestamp_list[i], TIME_FORMAT) - datetime.strptime(timestamp_list[i-1], TIME_FORMAT)).total_seconds() / 60)))

        timestamp_local_diff_list[:] = [x / max(timestamp_local_diff_list) if max(timestamp_local_diff_list) > 0 else 0 for x in timestamp_local_diff_list]
        ...
        for i in range(len(activity_list)):
            ...
            if partial_trace.__len__() > 1:
                dataset_dicts.append({
                    ...
                    "next_activity": activity_list[(i + window_size)] if ((i + window_size)< len(activity_list)) else "end",
...
def create_dictionaries(dataset):
    # creo delle liste contententi dei dizionari utili a creare la matrice finale
    # ogni dizionario avrà come chiave il nome della colonna nella matrice
    ...
    for _, row in dataset.iterrows():
        ...
        for time_instant, resource in enumerate(row[2]):
            resource_dict["resource_" + str(time_instant + 1)] = resource

        for time_instant, timestamp in enumerate(row[4]):
            time_dict["time_diff_" + str(time_instant + 1)] = timestamp
        #for time_instant, timestamp in enumerate(row[5]):
        ...
        target_dict_list.append({str(row[3]).replace(" ", ""): 1})
    ...
    resource_matrix = pd.DataFrame(resource_dict_list).fillna(int(0))
    target_matrix = pd.DataFrame(target_dict_list).fillna(0)  # .astype(int)

    return caseID_matrix, features_matrix, time_matrix, resource_matrix, target_matrix


def reorder_df_columns(first_matrix, second_df_matrix):
    first_columns = first_matrix.columns.tolist()

    second_df_matrix = second_df_matrix[first_columns]

    return second_df_matrix


def save_numpy_matrices(caseID_matrix, features_matrix, time_matrix, resource_matrix, targets_matrix, filename):
    final_matrix = caseID_matrix.join(features_matrix)
    #final_matrix = final_matrix.join(resource_matrix)
    final_matrix = final_matrix.join(time_matrix)
    #final_matrix.to_csv(DATASET_FOLDER_PATH + "provafinal.csv", index=False, header=True)
    final_matrix = final_matrix.to_numpy()

    targets_matrix = targets_matrix.to_numpy()

    np.save(filename + "_features.npy", final_matrix)
    np.save(filename + "_targets.npy", targets_matrix)

    return final_matrix, targets_matrix


def saveLSTMMatrices(features, avg_length, filename):
    LSTMFeaturesDict = {}
    ...
    # featues.shape[1]-1 perchè non considero la colonna caseID
    #LSTMFeaturesMatrix = np.zeros((features.shape[0], max_size, features.shape[1] - 1))
    LSTMFeaturesMatrix = np.zeros((features.shape[0], avg_length, features.shape[1] - 1))

    i = 0
    for key in LSTMFeaturesDict.keys():
        partialTraces = LSTMFeaturesDict[key]
        LSTMFeaturesRow = np.zeros((avg_length, features.shape[1] - 1))
        for partialTrace in partialTraces:
            LSTMFeaturesRow = np.delete(LSTMFeaturesRow, 0, 0)
            LSTMFeaturesRow = np.vstack([LSTMFeaturesRow, np.zeros((features.shape[1] - 1))])
            LSTMFeaturesRow[avg_length - 1] = partialTrace
            LSTMFeaturesMatrix[i] = LSTMFeaturesRow
            i += 1
    np.save(filename + "_LSTMfeatures.npy", LSTMFeaturesMatrix)

    #print("\n")
    #print(LSTMFeaturesMatrix.shape)
    #print("1")
    #print(LSTMFeaturesMatrix[0])
    #print("2")
    #print(LSTMFeaturesMatrix[1])
    #print("3")
    #print(LSTMFeaturesMatrix[2])


def createImages(path, x_train, y_train, x_test, y_test, classes, filename):
    param = {"Max_A_Size": 16, "Max_B_Size": 16, "Dynamic_Size": False, 'Metod': 'tSNE', "ValidRatio": 0.1, "seed": 180,
             "dir": path + "/", "Mode": "CNN2",  # Mode : CNN_Nature, CNN2
    ...
    data = {"Xtrain": pd.DataFrame(x_train), "Classification": pd.DataFrame(y_train), "class": classes, "Xtest": pd.DataFrame(x_test), "Ytest": pd.DataFrame(y_test)}

    model = DeepInsight_train_norm.train_norm(param, data, norm=False, filename=filename)
